...
@unittest.skipIf(not os.path.exists(schemadir), "mdstudio_haddock package not installed")
class TestHaddockDataModelBlockAdd(UnittestPythonCompatibility):
    ...
    def test_data_block_new(self):
        ...
        self.assertDictEqual(parsed['project'], write_pydata(new.query_nodes(key='project')))
...
class TestHaddockDataModelBlockEdit(UnittestPythonCompatibility):
...
class TestHaddockDataModelHelpers(UnittestPythonCompatibility):
    ...
    def test_project_save(self):
        ...
        # Test equality by key name as the import/export order differs making
        # it difficult to compare node/edge keys
        self.assertItemsEqual(project.keys(), self.web.keys())
        self.assertEqual(len(project.edges), len(self.web.edges))
